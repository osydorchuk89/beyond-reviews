...
exports.movieRouter.get("/:movieId/reviews", async (req, res) => {
    const { movieId } = req.params;
    try {
        ...
        if (movieReviews) {
            const sortedMovieReviews = movieReviews.sort((a, b) => b.likedBy.length - a.likedBy.length);
...
exports.movieRouter.post("/:movieId/reviews", async (req, res) => {
    const { rating, text } = req.body;
    const validationResult = schemas_1.ReviewSchema.safeParse({
        rating,
        text,
    });
    if (!validationResult.success) {
        ...
        try {
            ...
            if (movie) {
                ...
                const newAvgRating = movieReviews.reduce((acc, item) => acc + item.rating, 0) /
                    movieReviews.length;
...
exports.movieRouter.put("/:movieId/reviews/:reviewId", async (req, res) => {
